"""CFPB enforcement actions connector.

Fetches Consumer Financial Protection Bureau enforcement actions from
the CFPB RSS feed. CFPB enforcement often precedes private class actions.
"""
...
_RSS_URL = "https://www.consumerfinance.gov/policy-compliance/enforcement/rss/"
...
class CFPBEnforcementConnector(BaseConnector):
    """CFPB enforcement actions via RSS.

    Covers enforcement press releases, consent orders, and consumer
    complaint trend analysis relevant to financial class actions.
    """

    connector_id = "federal.cfpb.enforcement"
    source_tier = SourceTier.FEDERAL
    source_label = "CFPB Enforcement Actions"
    ...
    async def fetch_latest(
        ...
        if not terms:
            logger.warning("cfpb_enforcement.no_query_terms")
            return []
        ...
        async with create_http_client() as client:
            ...
            try:
                ...
                logger.warning("federal_cfpb_enforcement.fetch_error", exc_info=True)
                return []
        ...
        logger.info("cfpb_enforcement.fetched", count=len(records))
        return records

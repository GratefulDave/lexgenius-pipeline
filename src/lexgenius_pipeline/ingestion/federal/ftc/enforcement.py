"""FTC enforcement actions connector.

Fetches Federal Trade Commission enforcement actions from the FTC RSS feed,
covering consumer protection, antitrust, privacy, and competition cases.
FTC enforcement often precedes private class action litigation.
"""
...
_RSS_URL = "https://www.ftc.gov/news-events/news/press-releases/rss"
...
class FTCEnforcementConnector(BaseConnector):
    """FTC enforcement actions via RSS feed.

    Covers consumer protection enforcement, antitrust actions, privacy
    enforcement, and competition-related consent orders.
    """

    connector_id = "federal.ftc.enforcement"
    source_tier = SourceTier.FEDERAL
    source_label = "FTC Enforcement Actions"
    ...
    async def fetch_latest(
        ...
        if not terms:
            logger.warning("ftc_enforcement.no_query_terms")
            return []
        ...
        async with create_http_client() as client:
            ...
            try:
                ...
                logger.warning("federal_ftc_enforcement.fetch_error", exc_info=True)
                return []
        ...
        logger.info("ftc_enforcement.fetched", count=len(records))
        return records

...
_SETTLEMENTS_URL = f"{_BASE_URL}/settlements"
_DEADLINE_RE = re.compile(
    r"(?:deadline|claim\s+(?:by|before|deadline))[:\s]*(\w+\s+\d{1,2},?\s+\d{4})",
    re.IGNORECASE,
)
_AMOUNT_RE = re.compile(r"\$[\d,]+(?:\.\d+)?\s*(?:million|billion|M|B)?", re.IGNORECASE)
...
def _parse_iso_date(date_str: str) -> datetime:
    try:
        ...
        logger.warning("classaction_org.unparseable_date", date_str=date_str)
        return datetime.now(tz=timezone.utc)


class ClassActionOrgConnector(BaseConnector):
    connector_id = "commercial.classaction_org"
    source_tier = SourceTier.COMMERCIAL
    source_label = "ClassAction.org"
    ...
    async def fetch_latest(
        ...
        async with create_http_client() as client:
            ...
            for heading in soup.find_all(["h2", "h3"]):
                ...
                excerpt_el = heading.find_next(
                    "div",
                    class_=re.compile(r"entry-content|excerpt|summary|field-body"),
                )
        ...
        logger.info("classaction_org.fetched", count=len(records))
        return records
